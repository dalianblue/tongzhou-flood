...
<div><b style="color:var(--txt)">洪水事件验证（四事件两层淹没线全自洽）</b><br>
· 2026（校准年）：4 次过阈全中，首黄提前 19/48/39/48h，6 次过淹时刻（含 4 次复淹）全亮灯<br>
· 2024（官方简报，<b>无台风</b>）：干流漫溢型，富阳站 9.62m 超漫溢线 → 全岛撤离；上游首警领先岛峰 23.5h，官方"保证水位"红警与岛进水同时<br>
· 2025（独立验证数据，台风蝴蝶 618km 弱复合）：干流 8.07m 持续 30h 擦线未淹——首红提前 14h（宁撤勿漏），复合因子弱一档故未淹<br>
· 实地校正：新桐乡站 = 岛上实测水深 + 2.13m（σ=0.06m）——干流漫溢线 8.1~9.2m / 复合进水线 6.5~7.0m<br>
<b style="color:var(--txt)">稳健性评估（外部建议量化）</b><br>
· 阈值敏感性：渌渚镇 0.3~0.5 为平台，<b>0.5→0.6 是断崖</b>（8 月提前量 39h→15h）——失配退回 15h 仍过 ≥6h 自检线，后果有界<br>
· 代价敏感（漏报 5×权重）：现行 5×0 漏报 + 150h 黄误报；漏报一次远贵于误报一小时，故取平台左缘低阈值<br>
· 已否决：『xt≥6.5 持续 6h 升红』（2026 三事件零增益，2025 持续 30h 未淹反证——时长不是判别子）<br>
· 三因子（台风≤500km+雨≥80mm+支流涨≥0.5）不做等级，作复合进水型确认标识</div>
...
def main():
    ...
    for attempt in range(2):
        try:
            ...
            if e.errno != 48 or attempt:
                raise
            # 端口被占: 若是本项目的旧 dashboard 进程则自动接管, 否则提示退出
            ...
            for pid in pids:
                ...
                if "dashboard.py" in cmd:
                    ours.append(pid)
            if not ours:
                print(f"端口 {a.port} 被非 dashboard 进程占用 (PID {','.join(pids) or '?'})，"
                      f"请换端口: python3 dashboard.py --port {a.port + 1}")
                sys.exit(1)
            print(f"接管端口 {a.port}: 停止旧实例 PID {','.join(ours)} …")
            for pid in ours:
